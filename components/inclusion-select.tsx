...
// Shared Excluded / Included (Free) / Included (Price) picker, with the
// price field only shown when a price is actually meaningful (i.e. not for
// a free inclusion). Used for the Excursion library default, per-trip
// excursion overrides, and per-day itinerary inclusion.
    ...
    <div className={`flex items-center gap-2 ${className ?? ''}`}>
      <Select value={status} onValueChange={val => onStatusChange(val as InclusionStatus)}>
        ...
        <SelectContent className="bg-[#111111] border-[#1c1c1c]">
          ...
          <SelectItem value="included_free" className="text-zinc-300 focus:bg-zinc-800">Included (Free)</SelectItem>
          <SelectItem value="included_paid" className="text-zinc-300 focus:bg-zinc-800">Included (Price)</SelectItem>
        </SelectContent>
      </Select>
      {showPrice && (
        <Input
